...
class Cyplot:
    ...
    def set_data(self, data, index, ylabel, add=False):
        ...
        if self.ptype == 'PCA':
            ...
            if index is not None:
                X = data[columns!=index].values
                Y = data[columns==index].values
            else:
                X = data.values[:,:-1]
                Y = data.values[:,-1]
        ...
        if isinstance(data, pd.Series):
            ...
            if self.dims is not None and len(self.dims)>1:
                self.data = data.set_index(self.dims[0])
        ...
        if ylabel is not None:
            ...
            # self.legends
            self.ylabel = '' 
            if self.dims is not None and len(self.dims)==2:
    ...
    def create_fig(self, ts):
        ...
        if self.ptype =='PCA':
            ...
            pplt = Scatter(x=self.xd, y=self.yd, scales={'x': self.xScale, 'y': self.yScale}, color = self.colors)#labels=self.legends,
